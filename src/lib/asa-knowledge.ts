...
export function getAsaKnowledgeBase(): AsaKnowledgeEntry[] {
  ...
  const entries: AsaKnowledgeEntry[] = [
    ...
    },
    {
      ...
      buildAnswer: () =>
        `**${name}** is an independent **${SCHOOL_INFO.phase.toLowerCase()}** in **${SCHOOL_INFO.suburb}**. We focus on disciplined, nurturing education from **Grade R through Grade 7**. Principal: **${SCHOOL_INFO.principal}** (${SCHOOL_INFO.principalYear}). About **${SCHOOL_INFO.totalLearners}** learners and **${SCHOOL_INFO.totalEducators}** educators.`,
    ...
    },
    {
      ...
      buildAnswer: () =>
        `You can **apply online** on Admissions (step-by-step form) or **download the printable PDF** application with our logo and contact details.\n\nBoth go to **${SCHOOL_INFO.email}**. After that: admissions review → **interview or assessment** → **enrolment** if accepted.\n\nPrefer to talk first? Call or use the contact form.`,
      navigateTo: "/admissions#asa-apply-online",
    ...
    },
    {
      ...
      buildAnswer: () =>
        `Our **Sports** page covers athletics, soccer, netball, PE, and inter-house fixtures — plus what learners need for practices and match days.`,
      navigateTo: "/sports#asa-sports",
      linkLabel: "Sports page",
    },
    {
      id: "culture",
      keywords: [
      ...
      buildAnswer: () =>
        `Our **Culture** page describes heritage programmes, music, dance, languages, and upcoming cultural events at ${shortName}.`,
      navigateTo: "/culture#asa-culture",
    ...
    },
    {
      ...
      buildAnswer: () =>
        `Our **Code of Conduct** covers discipline, attendance, and family responsibilities. For uniform items and prices, see the **Uniform Catalog**.`,
  ...
  if (hasSisterSchoolLink()) {
    entries.push({
      ...
      buildAnswer: () =>
        `We also have **${SISTER_SCHOOL_LINK.label}** — a separate campus site you can open from the menu. This site covers **${SCHOOL_INFO.suburb}**.`,
